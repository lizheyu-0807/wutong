...
const checks = [
  ...
  ['patient scan intake captures appointment date and time', app.includes('examDate') && app.includes('examTime') && app.includes('检查日期') && app.includes('检查时间')],
  ...
  ['patient home removes the legacy identity preparation card', !app.includes('home-clean-card') && !app.includes('的检查准备</h1>')],
  ['patient home title uses registered appointment date time and project', app.includes('formatPatientAppointmentTitle') && app.includes('getPatientExamDate') && app.includes('getPatientExamTime') && app.includes('intake?: PatientIntakeRecord | null') && app.includes('appointment-title')],
  ['patient home exposes compact dual flow bracelet selectors', app.includes('function FlowChainSelector') && app.includes('flow-chain-switcher') && app.includes('flow-chain-card') && app.includes('flow-chain-mini-stack') && css.includes('.flow-chain-switcher') && css.includes('.flow-chain-card')],
  ...
  ['patient home supports separate pre-hospital and hospital visit card chains', app.includes('preHospitalNodes') && app.includes('hospitalVisitNodes') && app.includes('activeFlow') && app.includes('医院就诊') && app.includes('switchToHospitalVisit')],
